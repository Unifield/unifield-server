        ...
        for rfq in self.browse(cr, uid, ids, context=context):
            for line in rfq.order_line:
                ...
                if line.price_unit and line.product_id:
                    ...
                    info_99_list = info_obj.search(cr, uid,
            ...
            if ((purchase.order_type == 'regular' and purchase.partner_id.partner_type in ('internal', 'esc')) or \
                ...
                if stock_pickings:
                    ...
                    for sp in stock_pickings:
                        if inv_obj.any_exists(cursor, user, [('picking_id',
                            '=', sp)]):
                            sp_ids.remove(sp)
                    if sp_ids:
    ...
    def _po_from_x(self, cr, uid, ids, field_names, args, context=None):
        ...
        for po_data in self.read(cr, uid, ids, ['order_line'], context=context):
            ...
            if pol_ids:
                ...
                if proc_ids:
                    # po_from_ir
                    sol_exists = sol_obj.any_exists(
                        cr, uid,
                        [('procurement_id', 'in', proc_ids)], context=context)
                    res[po_data['id']]['po_from_ir'] = sol_exists
                    # po_from_fo
                    sol_exists = sol_obj.any_exists(
                        cr, uid,
                        ...
                        context=context)
                    res[po_data['id']]['po_from_fo'] = sol_exists
        return res
...
stock moves which are already processed : '''
    ...
    def all_po_confirmed(self, cr, uid, ids, context=None):
        ...
        if all_so_ids:
            ...
            if exp_sol_obj.any_exists(cr, uid,
                    [('order_id', 'in', all_so_ids)], context=context):
                return False

            if sol_obj.any_exists(cr, uid,
        ...
        for order in self.browse(cr, uid, ids):
            ...
            if todo2:
                ...
                for move in move_obj.browse(cr, uid, sm_ids, context=context):
                    ...
                    backmove_exists = move_obj.any_exists(cr, uid,
                            [('backmove_id', '=', move.id)])
                    if move.state != 'done' and not backmove_exists and not move.backmove_id:
...
class purchase_order_line(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        other_lines_exists = self.any_exists(cr, uid, [('order_id', '=', order_id),
        ...
        if (other_lines_exists and stages and order.state != 'confirmed'):
    ...
    def update_origin_link(self, cr, uid, origin, context=None):
        ...
        so_ids = so_obj.search(cr, uid, [('name', '=', origin), ('state', 'in',
            ('sourced', 'progress', 'manual'))], context=context)
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if not sol_ids and line.origin:
                ...
                for origin in so_obj.read(cr, uid, origin_ids, ['order_line'], context=context):
                    exp_sol_exists = exp_sol_obj.any_exists(cr, uid, [('order_id',
                        '=', origin['id']), ('po_line_id', '!=', line.id)],
                        context=context)
                    if not origin['order_line'] and not exp_sol_exists:
    ...
    def fake_unlink(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if not self.pool.get('sale.order.line.cancel').any_exists(cr, uid, [
    ...
    def on_change_origin(self, cr, uid, ids, origin, procurement_id=False, partner_type='external', context=None):
        ...
        if not procurement_id and origin:
            ...
            if not self.pool.get('sale.order').any_exists(cr, uid, domain,
                    context=context):
    ...
    def product_id_on_change(self, cr, uid, ids, pricelist, product, qty, uom,
        ...
        if product and not res.get('value', {}).get('price_unit', False) and all_qty != 0.00 and qty != 0.00:
            ...
            info_prices = partner_price.search(cr, uid, domain_cur, order='sequence asc, min_quantity asc, id desc', context=context)
            if not info_prices:
                info_prices = partner_price.search(cr, uid, domain, order='sequence asc, min_quantity asc, id desc', context=context)
    ...
    def price_unit_change(self, cr, uid, ids, fake_id, price_unit, product_id,
        ...
        order = self.pool.get('purchase.order').browse(cr, uid, order_id, context=context)
        other_lines_exists = self.any_exists(cr, uid,
                [('id', '!=', fake_id),
                 ('order_id', '=', order_id),
                 ('product_id', '=', product_id),
        ...
        if not change_price_ok or (other_lines_ecists and stages and order.state != 'confirmed' and not context.get('rfq_ok')):
...
class purchase_order_line_unlink_wizard(osv.osv_memory):
    ...
    def _get_last_line(self, cr, uid, ids, field_name, args, context=None):
        ...
        for wiz in self.browse(cr, uid, ids, context=context):
            ...
            if not pol_obj.any_exists(cr, uid, [('id', '=', wiz.line_id.id)],
                    context=context):
                continue
...
class purchase_order_cancel_wizard(osv.osv_memory):
    ...
    def _get_last_lines(self, cr, uid, order_id, context=None):
        ...
        exp_sol_exists = exp_sol_obj.search(cr, uid,
                                         [('order_id', 'in', po_so_ids),
                                          ('po_id', '!=', order_id)],
                                         context=context)

        if not exp_sol_exists and not po_ids:
            return True
