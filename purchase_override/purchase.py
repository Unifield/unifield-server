    ...
    def _update(self, cr, uid, id, product_qty, price=0.00, context={}, no_update=False):
        ...
        if new_price:
            ...
            for order_line in line.order_line_ids:
                product_qty += order_line.product_qty
            values.update({'product_qty': product_qty, 'price_unit': price})
...
class purchase_order_line(osv.osv):
    ...
    def link_merged_line(self, cr, uid, vals, product_id, order_id, product_qty, uom_id, price_unit=0.00, context={}):
        ...
        if not merged_ids:
            ...
            res_merged = line_obj._update(cr, uid, merged_ids[0], product_qty, price_unit, context=context, no_update=True)
            if not res_merged or not res_merged[1]:
                vals['merged_id'] = line_obj.create(cr, uid, new_vals, context=context)
            else:
    ...
    def _update_merged_line(self, cr, uid, line_id, vals, context={}):
        ...
        if vals and line_id:
            ...
            if 'product_qty' in vals and line.product_qty != vals['product_qty']:
                ...
                if res_merged and res_merged[1]:
                    vals.update({'price_unit': res_merged[1]})
            if 'price_unit' in vals and line.price_unit != vals['price_unit']:
                res_merged = merged_line_obj._update(cr, uid, line.merged_id.id, vals.get('product_qty', 0.00)-line.product_qty, vals['price_unit'], context=context)
