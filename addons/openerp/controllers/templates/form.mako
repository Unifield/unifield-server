...
<%def name="content()">
    ...
    <script type="text/javascript">
        jQuery(document).ready(function () {
            ...
            % endif
            % if loading_id:
            jQuery.fancybox({'href': "/openerp/progressbar?model=${form.screen.model}&id=${form.screen.id}&job_id=${loading_id}", 'modal': true, 'onClickClosed': function() {window.editRecord(${form.screen.id})}});
            % endif
        })
    </script>
</%def>
