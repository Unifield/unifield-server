% if editable:
    <span class="char">
        <input type="${password and 'password' or 'text'}" size="1"
            ...
            ${py.attrs(attrs, kind=kind, maxlength=size, value=value, required=required and 1 or 0, fld_readonly=readonly and 1 or 0, hide_default_menu=hide_default_menu and 1 or 0)}/>
        % if translatable:
