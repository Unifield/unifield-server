...
class purchase_order_integration_report_xls(WebKitParser):
    def __init__(self, name, table, rml=False, parser=report_sxw.rml_parse, header='external', store=False):
        WebKitParser.__init__(self, name, table, rml=rml, parser=parser, header=header, store=store)
    ...
    def create_single_pdf(self, cr, uid, ids, data, report_xml, context=None):
        ...
        report_xml.header= " "
        report_xml.webkit_header.html = "${_debug or ''|n}"
        return super(purchase_order_integration_report_xls, self).create_single_pdf(cr, uid, ids, data, report_xml, context)

    def create(self, cr, uid, ids, data, context=None):
        ids = getIds(self, cr, uid, ids, context)
        a = super(purchase_order_integration_report_xls, self).create(cr, uid, ids, data, context)
        return (a[0], 'xls')

purchase_order_integration_report_xls('report.po.integration_xls','purchase.order','addons/msf_supply_doc_export/report/report_po_integration_xls.mako')
