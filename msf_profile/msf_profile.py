...
class patch_scripts(osv.osv):
    ...
    def us_918_patch(self, cr, uid, *a, **b):
        ...
        if sync_server_update_module:  # if we are on a server instance
            ...
            updates_to_modify = sync_server_update_module.search(
                cr, uid, [('values', 'like', '%sd.ZMW%')],)
            for update in sync_server_update_module.browse(cr, uid,
                                                           updates_to_modify,
                                                           context={}):
                ...
                sync_server_update_module.write(cr, uid, update.id, vals)
        ...
        else:  # if we are on a client instance
