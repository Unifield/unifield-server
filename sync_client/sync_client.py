...
class entity(osv.osv, Thread):
    ...
    """
        Pull update
    """
    def pull_update(self, cr, uid, all=False, context=None):
        if not context:
            context = {}
        entity = self.get_entity(cr, uid, context)
        ...
        try:
            ...
            self.retreive_update(cr, uid, all, context=context)
            cr.commit()
    ...
    def retreive_update(self, cr, uid, all=False, context=None):
        entity = self.get_entity(cr, uid, context)
        last = False
        ...
        while not last:
            proxy = self.pool.get("sync.client.sync_server_connection").get_connection(cr, uid, "sync.server.sync_manager")
            res = proxy.get_update(entity.identifier, last_seq, offset, max_packet_size, max_seq, all, context)
            if res and res[0]:
