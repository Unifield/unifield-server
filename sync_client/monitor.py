...
class MonitorLogger(object):
    ...
    def close(self):
        ...
        # UTP-1200: Moved the following block to the method: update_sale_purchase_logger and call this method
        # right after the message pull is done, not need to wait until the end of sync since it's not relevant to the push
        # but also to avoid unnecessary error caused by the "in progress" issue (fixed but better to avoid)
