...
class WizardCurrencyrevaluation(osv.osv_memory):
    ...
    def on_change_fiscalyear_id(self, cr, uid, ids, method, fiscalyear_id):
        ...
        if method in ['liquidity_month', 'liquidity_year']:
            if fiscalyear_id:
                ...
                # Set the next period as the 'Entry period' + the entry date
                #next_period_date = datetime.datetime.strptime(
                #    period.date_start, '%Y-%m-%d') + relativedelta(months=1)
                #next_period_ids = period_obj.get_period_from_date(
    ...
    def _compute_unrealized_currency_gl(self, cr, uid,
        ...
        # Check if reval_foreign_balance exists
        if foreign_balance or balances.get('reval_foreign_balance', 0.0) != 0.0:
            ctx_rate['revaluation'] = True
            # Use reval_foreign_balance to fix booking balance problems
            adjusted_balance = currency_obj.compute(
                cr, uid, currency_id, cp_currency_id, foreign_balance,
                context=ctx_rate)
            # Substract reval lines from amount
            unrealized_gain_loss =  adjusted_balance - balance - balances.get('reval_foreign_balance', 0.0)
