...
class product_likely_expire_report_xls(SpreadsheetReport):
    def __init__(self, name, table, rml=False, parser=report_sxw.rml_parse,
        header='external', store=False):
        super(product_likely_expire_report_xls, self).__init__(name, table,
            rml=rml, parser=parser, header=header, store=store)

    def create(self, cr, uid, ids, data, context=None):
        a = super(product_likely_expire_report_xls, self).create(cr, uid, ids,
        data, context)
        return (a[0], 'xls')
