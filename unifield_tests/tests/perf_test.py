...
class PerfTest(UnifieldTest):
    ...
    def setUp(self):
        ...
        self.pick_obj = db.get('stock.picking')
        self.move_obj = db.get('stock.move')
        self.prod_obj = db.get('product.product')
        self.product_ids = self.prod_obj.search([('batch_management', '=', False), ('perishable', '=', False)])
    ...
    def create_order(self, db):
        ...
        for i in range(50):
            line_values.update({
                ...
                'product_uom_qty': random.randint(1, 200),
            })
            self.order_line_obj.create(line_values)
...
The state of the generated PO is %s - Should be 'confirmed'""" % po_state)
    ...
    def process_in(self, in_to_process):
        in_proc_obj = self.used_db.get('stock.incoming.processor')
        if isinstance(in_to_process, (int, long)):
            in_to_process = [in_to_process]

        for incoming in in_to_process:
            in_proc_id = self.pick_obj.action_process(in_to_process)['res_id']
            in_proc_obj.copy_all(in_proc_id)
            in_proc_obj.do_incoming_shipment([in_proc_id])

    def run_process(self):
        fo_id, fo_lines, po_ids, po_line_ids = self.order_source_all_one_po(self.used_db)
        self._validate_po(self.used_db, po_ids)
        self._confirm_po(self.used_db, po_ids)

        # Search IN
        in_ids = self.pick_obj.search([('purchase_id', 'in', po_ids)])
        import time
        start = time.time()
        self.process_in(in_ids)
        end = time.time() - start
        return end

    def test_perf_30(self):
        total_time = 0.00
        x10_time = 0.00
        for i in range(100):
            exec_time = self.run_process()
            x10_time += exec_time
            total_time += exec_time
            if i != 0 and i % 10 == 0:
                print ('%s :: Exec time: %s' % (i, x10_time))
                x10_time = 0.00
