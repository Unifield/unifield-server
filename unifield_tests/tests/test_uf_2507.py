...
class UF2507ResourcingTest(UnifieldTest):

    def uf_2507(self, cls_test):
        """
        ...
        """
        order_id, order_line_ids, po_ids, po_line_ids = cls_test.\
        ...
        self.assert_(
            cls_test._get_number_of_valid_lines(db, order_id) == 3,
        ...
        5/ Validate and confirm the PO
        """
        cls_test._validate_po(db, po_ids)
        cls_test._confirm_po(db, po_ids)

        """
        6/ Check the number of lines in the FO/IR
        """
        # Check the FO line has been added
        fo_lines_nb = cls_test._get_number_of_valid_lines(db, order_id)
        self.assert_(
            fo_lines_nb == 4,
            "The line has not been added well on the order (%s - should be 4)" % fo_lines_nb,
        )

    def test_fo_uf_2507(self):
        cls_test = ResourcingTest(pr=False)
        return self.uf_2507(cls_test)

    def test_ir_uf_2507(self):
