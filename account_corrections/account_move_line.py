...
#        return success_move_line_ids
    ...
    def corrected_upstream_marker(self, cr, uid, ids, context=None):
        ...
        if company and company.instance_id and company.instance_id.level in ['section', 'coordo']:
            ...
            # move_ids = [x.get('move_id')[0] for x in self.read(cr, uid, ids, ['move_id'], context=context) if x.get('move_id')]
            if ids and ids[0] != False:
                for x in self.read(cr, uid, ids, ['move_id'], context=context):
                    if x.get('move_id'):
                        move_ids = [x.get('move_id')[0]]
                if move_ids:
