...
# original code from https://github.com/uber/py-find-injection
    ...
    def check_execute(self, node):
        if isinstance(node, ast.BinOp):
            ...
            if assignment is not None:
                #print ast.dump(assignment.value)
                if not self.ignore_sql_check(assignment.value):
                    return self.check_execute(assignment.value)

    def visit_Call(self, node):
        function_name = stringify(node.func)
        if function_name.lower() in ('cr.execute', 'cursor.execute') and not self.ignore_sql_check(node):
...
def main():
    ...
    for fname in args.files:
        ...
        if os.path.isdir(fname):
            for root, directories, filenames in os.walk(fname):
