...
class _BaseFile(list):
    ...
    def ordered_metadata(self):
        ...
        for data in natural_sort(list(metadata.keys())):
            value = metadata[data]
            ordered_data.append((data, value))
        return ordered_data
...
class POEntry(_BaseEntry):
    ...
    def __unicode__(self, wrapwidth=78):
        ...
        if not self.obsolete and self.occurrences:
            ...
            # Simply add all occurences (no wrap) to avoid completely messing
        ...
        for occurrence in occurrences:
            if occurrence != '':
                try:
                    ...
                    if not line.isdigit():
                        # https://bitbucket.org/izi/polib/issues/90/bug-when-parsing-occurrences-with-no-line
                        fil = occurrence
                        line = ''
