...
class PhysicalInventory(osv.osv):
    ...
    def _inventory_totals(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for inventory in inventories:
            ...
            total['discrepancy_lines_percent'] = 100.0 * total['discrepancy_lines_number'] / total['inventory_lines_number'] if total['inventory_lines_number'] else 0.0
    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        for inv in self.read(cr, uid, ids, ['signature_id', 'signature_available', 'signed_off_line'], context=context):
            if inv['signature_id'] and inv['signature_available'] and not inv['signed_off_line'] and\
                ...
                raise osv.except_osv(_('Error'), _('Both the Warehouse and Supply Responsible roles must be signed in order to close the Inventory'))
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        for inv in self.read(cr, uid, ids, ['counting_line_ids', 'discrepancy_line_ids', 'date', 'name', 'state',
            ...
            if inv['signature_id'] and inv['signature_available'] and not inv['signed_off_line'] and \
                ...
                raise osv.except_osv(_('Error'), _('The Warehouse Responsible role must be signed in order to confirm the Inventory'))
            move_ids = []
