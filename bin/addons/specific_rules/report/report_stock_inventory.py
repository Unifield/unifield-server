...
location will be shown.""",
        ...
        'display_0': fields.boolean(string='Include products with stock <= 0 with movements'),
...
class parser_report_stock_inventory_xls(report_sxw.rml_parse):
    ...
    def getLines(self):
        ...
        if not report.product_list_id and report.display_0:
            ...
            self.cr.execute('''select distinct(product_id) from stock_move where state='done' and (location_id in %s or location_dest_id in %s) and date >= %s and date <= %s''', (values['location_ids'], values['location_ids'], from_date, to_date))
        ...
        bn_data = {}
        product_data = {}
        # fecth data with db id: for uom, product, bn ...
        ...
        for product in self.pool.get('product.product').browse(self.cr, self.uid, product_ids_to_fetch, fields_to_fetch=['default_code', 'uom_id', 'name', 'standard_price'], context=self.localcontext):
            ...
            if product.id not in res:
                res[product.id] = {
                    'sum_qty': 0,
                    'lines': { '': {'qty': 0, 'batch_id': False, 'expiry_date': '', 'location_ids': {}}
                               }
                }
