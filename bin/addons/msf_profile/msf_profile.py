...
class patch_scripts(osv.osv):
    ...
    def us_6905_manage_bned_switch(self, cr, uid, *a, **b):
        ...
        # set bn on no moves
        cr.execute('''select distinct(product_id) from stock_move m, product_product p where p.id = m.product_id and p.perishable='t' and p.batch_management='t' and m.prodlot_id is null and m.state = 'done' ''')
        ...
        for prod_id in cr.fetchall():
            ...
            cr.execute("update stock_move set hidden_batch_management_mandatory='t', hidden_perishable_mandatory='f', prodlot_id=%s, expired_date=%s, old_lot_info='US-6905 BN set'||E'\n'||COALESCE(old_lot_info, '') where product_id=%s and prodlot_id is null and state in ('done', 'cancel')", (batch_id, fake_ed, prod_id))

        # set ed on no moves
        cr.execute('''select distinct(product_id) from stock_move m, product_product p where p.id = m.product_id and p.perishable='t' and p.batch_management='f' and m.prodlot_id is null and m.state = 'done' ''')
