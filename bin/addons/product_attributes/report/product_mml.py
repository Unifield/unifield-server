...
class product_mml(XlsxReportParser):
    ...
    def generate(self, context=None):
        ...
        sheet.title = _('MML Products')

        sheet.merged_cells.ranges.append("A1:F1")
        sheet.append([self.cell_ro(_('MML Products'), 'title_style')])
        ...
        sheet.append([
            ...
            self.cell_ro(_("Mission / Project\nRestriction"), 'header_row'),
            self.cell_ro(_('Use Comments'), 'header_row'),
        ...
        while True:
            prod_ids = prod_obj.search(self.cr, self.uid, [('oc_validation', '=', True)], limit=page_size, offset=offset, context=context)
            if not prod_ids:
                break
            offset += page_size
            ...
            for prod in prod_obj.browse(self.cr, self.uid, prod_ids, fields_to_fetch=['code', 'name', 'standard_ok', 'restrictions_txt', 'oc_comments', 'oc_subscription', 'state_ud', 'state', 'msl_status', 'mml_status'], context=context):
                sheet.append([
                    ...
                    self.cell_ro(prod.oc_comments or '', 'row'),
        ...
        sheet.auto_filter.ref = "A5:J5"

XlsxReport('report.report.product_mml', parser=product_mml, template='addons/product_attributes/report/product_mml.xlsx')
