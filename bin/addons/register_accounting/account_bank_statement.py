...
#            partner_type = ','.join([str(st_line.third_parties._table_name), str(st_line.third_parties.id)])
    ...
    def write(self, cr, uid, ids, values, context=None):
        ...
        if state == 'hard':
            if values.get('partner_move_ids') or \
                    ...
                    list(values.keys()) == ['has_a_counterpart_transfer'] :
    ...
    def posting(self, cr, uid, ids, postype, force=False, context=None):
        """
        ...
        for absl in sorted(absls, key=lambda x: x.sequence_for_order):
            ...
            if postype == 'hard' and \
                ...
                if absl.sequence_for_reference:
                    init_ref = absl.sequence_for_reference
                else:
    ...
    def button_duplicate(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if line.account_id and line.account_id.name in ('Outstanding Cheques', 'Outstanding checks')  and line.from_import_cheque_id and line.from_import_cheque_id.id is not None:
                raise osv.except_osv(_('Warning'), _("You can't duplicate imported cheque line in bank register."))
            default_vals = ({
                'name': '(copy) ' + line.name,
                'cheque_number': None,
            })
