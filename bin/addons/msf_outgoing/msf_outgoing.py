...
class shipment(osv.osv):
    ...
    def _get_fake(self, cr, uid, ids, name, args, context=None):
        '''
        Fake method for 'sale_id', 'product_id' field
        '''
        res = {}
        if not ids:
            return res
    ...
    _columns = {
        ...
        'sale_id': fields.function(_get_fake, method=True, type='many2one', relation='sale.order', string='Origin', help='FO or IR to find in the Pack Families', store=False, domain=[('procurement_request', 'in', ['t', 'f'])], readonly=True),
        'product_id': fields.function(_get_fake, method=True, type='many2one', relation='product.product', string='Product', help='Product to find in the Pack Families', store=False, readonly=True),
