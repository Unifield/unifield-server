...
class audittrail_rule(osv.osv):
    ...
    def subscribe(self, cr, uid, ids, *args):
        ...
        for thisrule in self.browse(cr, uid, ids):
            ...
            search_view_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'msf_audittrail',
                                                                                 'view_audittrail_log_line_search')
            val = {
            ...
            if thisrule.object_id.model in ('purchase.order', 'purchase.order.line'):
                ...
                val['domain'] = "[('fct_object_id','=', %d), ('fct_res_id', 'in', active_ids)]" % (
                thisrule.object_id.id,)
            ...
            if action_search:
                if len(action_search) > 1:
                    ...
                    logger.warn(
                        'There is already %s ir.actions.act_window matching the domain %r, the first one will be updated' % (
                        len(action_search), search_domain))
            ...
            obj_model.ir_set(cr, uid, 'action', keyword, 'View_log_' + thisrule.object_id.model,
                             [thisrule.object_id.model], value, replace=True, isobject=True, xml_id=False,
                             view_ids=view_ids)
            cr.execute('update ir_values set sequence = 99999 where model=%s and key=\'action\' and name=%s',
                       (thisrule.object_id.model, 'View_log_' + thisrule.object_id.model))
            # End Loop

        # Check if an export model already exist for audittrail.rule
        export_ids = self.pool.get('ir.exports').search(cr, uid, [('name', '=',
                                                                   'Log Lines'),
                                                                  ('resource', '=', 'audittrail.log.line')], limit=1,
                                                        order='NO_ORDER')
        if not export_ids:
            export_id = self.pool.get('ir.exports').create(cr, uid, {'name': 'Log Lines',
                                                                     'resource': 'audittrail.log.line'})
            fields = ['log', 'timestamp', 'sub_obj_name', 'method', 'field_description', 'old_value', 'new_value',
                      'user_id']
            for f in fields:
