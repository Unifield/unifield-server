...
class product_product(osv.osv):
    ...
    def compute_amc(self, cr, uid, ids, context=None, compute_amc_by_month=False, remove_negative_amc=False, rounding=True):
        ...
        to_date_str = min(datetime.now(), to_date_str)
        nb_months = self._get_date_diff(from_date_str, to_date_str)
        ...
        if context.get('amc_location_ids') or (not context.get('histo_dest_location_ids') and context.get('adjusted_rr_amc')):
            ...
            for x in cr.fetchall():
                ...
                dt_to_over = datetime.strptime(to_over, '%Y-%m-%d')
                dt_from_over = datetime.strptime(from_over, '%Y-%m-%d')
                    ...
                    if  x[3] is None:
                        ...
                        if compute_amc_by_month:
                            ...
                            while tmp_dt_from_over <= dt_to_over:
                                ...
                                last_period_day = tmp_dt_from_over + relativedelta(months=1, day=1)
                                ...
                                tmp_dt_from_over += relativedelta(months=1, day=1)

                    else:
                        adjusted_qty.setdefault(x[0], 0)
                        adjusted_qty_by_day = x[3]/float((datetime.strptime(x[2], '%Y-%m-%d') - datetime.strptime(x[1], '%Y-%m-%d')).days)
                ...
                for idx in [4, 6, 8]:
                    if x[idx] in list(res.keys()) and x[idx+1]:
                        ...
                        adjusted_qty_by_day = x[idx+1]/float((datetime.strptime(x[2], '%Y-%m-%d') - datetime.strptime(x[1], '%Y-%m-%d')).days)
