...
class account_invoice_line(osv.osv):
    ...
    def _check_automated_invoice(self, cr, uid, invoice_id, context=None):
        """
        Prevents the creation of manual inv. lines if the related invoice has been generated by a Supply workflow or via Sync.
        """
        if context is None:
            context = {}
        inv_obj = self.pool.get('account.invoice')
        if invoice_id:
            inv = inv_obj.browse(cr, uid, invoice_id, fields_to_fetch=['from_supply', 'synced', 'type', 'is_inkind_donation'], context=context)
            is_ivi_or_si = inv.type == 'in_invoice' and not inv.is_inkind_donation
            if context.get('from_inv_form') and (inv.from_supply or (is_ivi_or_si and inv.synced)):
