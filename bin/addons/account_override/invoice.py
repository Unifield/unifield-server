...
class account_invoice_line(osv.osv):
    ...
    def _check_invoice_from_supply(self, cr, uid, invoice_id, context=None):
        """
        Prevents the creation of manual inv. lines if the related invoice has been generated by a Supply workflow
        """
        if context is None:
            context = {}
        inv_obj = self.pool.get('account.invoice')
        if invoice_id:
            invoice = inv_obj.read(cr, uid, invoice_id, ['from_supply'], context=context)
            if context.get('from_inv_form') and invoice.get('from_supply'):
                raise osv.except_osv(_('Error'), _('This document has been generated by a Supply workflow. You can\'t add lines manually.'))
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not context:
            context = {}
        self._check_invoice_from_supply(cr, uid, vals.get('invoice_id'), context=context)
