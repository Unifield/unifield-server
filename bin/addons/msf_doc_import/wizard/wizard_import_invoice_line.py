        ...
        for wiz in self.browse(cr, uid, ids, context):
            ...
            for line_num, row in enumerate(row_iterator, start=1):
                ...
                if col_count != template_col_count:
                    self.add_message(_("Line %s: You should have exactly %s columns in this order: %s \n") % (
                ...
                try:
                    ...
                    if not account_ids:
                        self.add_error(_('Line %s. Account %s not found!') % (line_num, account_value))
                        continue
                    ...
                    if account.type_for_register == 'donation':
                        self.add_error(_('Line %s. Account %s is donation type which is forbidden.') % (line_num, account_value))
                        continue
                    ...
                    if account.is_analytic_addicted:
                        ...
                        if not line[header_index[_('Destination')]]:
                            self.add_error(_('Line %s. No destination specified!') % (line_num,))
                            continue
                        ...
                        if not destination_ids:
                            self.add_error(_('Line %s. Destination %s not found!') % (line_num,
                                                                                      line[header_index[_('Destination')]],))
                            continue
                        r_destination = destination_ids[0]
                        # Check Cost Center
                        if not line[header_index[_('Cost Center')]]:
                            self.add_error(_('Line %s. No cost center specified!') % (line_num,))
                            continue
                        ...
                        if not cc_ids:
                            self.add_error(_('Line %s. Cost Center %s not found!') % (line_num, line[header_index[_('Cost Center')]]))
                            continue
                        r_cc = cc_ids[0]
                        # Check Cost Center type
                        cc = analytic_obj.browse(cr, uid, r_cc, context)
                        if cc.type == 'view':
                            self.add_error(_('Line %s. %s is a VIEW type Cost Center!') % (line_num, line[header_index[_('Cost Center')]]))
                            continue
                        # Check Funding Pool
                        r_fp = msf_fp_id
                        if line[header_index[_('Funding Pool')]]:
                            ...
                            if not fp_ids:
                                self.add_error(_('Line %s. Funding Pool %s not found!') % (line_num, line[header_index[_('Funding Pool')]]))
                                continue
                            r_fp = fp_ids[0]
                    ...
                    self.add_message(_("Line %s in the Excel file: %s: %s\n") % (line_num, osv_name, osv_value))
                    cr.rollback()
                    continue
                except UnicodeEncodeError as e:
                    self.add_message(_("""Line %s in the Excel file, uncaught error: %s\n""") % (line_num, e))
