...
class wizard_import_po_simulation_screen(osv.osv):
    ...
    _columns = {
        ...
        'with_ad': fields.selection(selection=[('yes', 'Yes'), ('no', 'No')],
                                    string='File contains Analytic Distribution', required=True),
        # File information
        'file_to_import': fields.binary(string='File to import'),
        'filename': fields.char(size=64, string='Filename'),
        'filetype': fields.selection([('excel', 'Excel file'), ('xml', 'XML file')], string='Type of file',),
        ...
        'in_supplier_ref': fields.related('order_id', 'partner_ref', type='char', string='Supplier Reference', readonly=True),
        'in_dest_addr': fields.related('order_id', 'dest_address_id', type='many2one', relation='res.partner.address',
                                       string='Destination Address', readonly=True),
        'in_transport_type': fields.related('order_id', 'transport_type', type='selection', selection=TRANSPORT_TYPE,
                                            string='Transport mode', readonly=True),
        'in_notes': fields.related('order_id', 'notes', type='text', string='Header notes', readonly=True),
        'in_currency': fields.related('order_id', 'pricelist_id', type='relation', relation='product.pricelist',
                                      string='Currency', readonly=True),
        'in_ready_to_ship_date': fields.related('order_id', 'ready_to_ship_date', type='date', string='RTS Date', readonly=True),
        'in_shipment_date': fields.related('order_id', 'shipment_date', type='date', string='Shipment date', readonly=True),
        'in_amount_untaxed': fields.related('order_id', 'amount_untaxed', string='Untaxed Amount', readonly=True),
        'in_amount_tax': fields.related('order_id', 'amount_tax', string='Taxes', readonly=True),
        'in_amount_total': fields.related('order_id', 'amount_total', string='Total', readonly=True),
        'in_transport_cost': fields.related('order_id', 'transport_cost', string='Transport mt', readonly=True),
        'in_total_price_include_transport': fields.related('order_id', 'total_price_include_transport',
                                                           string='Total incl. transport', readonly=True),
        'nb_po_lines': fields.function(_get_po_lines, method=True, type='integer', string='Nb PO lines', readonly=True),
        # Import fiels
        'imp_supplier_ref': fields.char(size=256, string='Supplier Ref', readonly=True),
        'imp_transport_type': fields.selection(selection=TRANSPORT_TYPE, string='Transport mode', readonly=True),
        'imp_ready_to_ship_date': fields.date(string='RTS Date', readonly=True),
        'imp_shipment_date': fields.date(string='Shipment date', readonly=True),
        'imp_notes': fields.text(string='Header notes', readonly=True),  # UFTP-59
        'imp_message_esc': fields.text(string='Message ESC Header', readonly=True),
        'imp_amount_untaxed': fields.function(_get_totals, method=True, type='float', string='Untaxed Amount',
                                              readonly=True, store=False, multi='simu'),
        'imp_amount_total': fields.function(_get_totals, method=True, type='float', string='Total Amount',
                                            readonly=True, store=False, multi='simu'),
        'imp_total_price_include_transport': fields.function(_get_totals, method=True, type='float',
                                                             string='Total incl. transport', readonly=True, store=False,
                                                             multi='simu'),
        'amount_discrepancy': fields.function(_get_totals, method=True, type='float', string='Discrepancy',
                                              readonly=True, store=False, multi='simu'),
        'imp_nb_po_lines': fields.function(_get_import_lines, method=True, type='integer', string='Nb Import lines', readonly=True),
        'simu_line_ids': fields.one2many('wizard.import.po.simulation.screen.line', 'simu_id', string='Lines', readonly=True),
...
a valid transport mode. Valid transport modes: %s') % (transport_type, possible_type)
    ...
    a valid date. A date must be formatted like \'YYYY-MM-DD\'') % shipment_date
                ...
                # For auto-import
                if nb_rejected:
                    context['nb_rejected_po_vi_lines'] = nb_rejected
