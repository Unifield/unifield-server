...
a valid transport mode. Valid transport modes: %s') % (transport_type, possible_type)
    ...
    a valid date. A date must be formatted like \'YYYY-MM-DD\'') % shipment_date
                ...
                for x in xrange(first_line_index+1, len(values)+1):
                    ...
                    if product_id:
                        res_restrict, restrict_error_msg = prod_obj._test_restriction_error(cr, uid, [product_id], {'partner_id':  wiz.order_id.partner_id.id}, context=dict(context, noraise=True))
                        if restrict_error_msg:
                            not_ok = True
                            file_line_error.append(tools.ustr(restrict_error_msg))
                            values_line_errors.append(_('Line %s of the PO: %s') % (line_number, tools.ustr(restrict_error_msg)))
                    # UoM
                    if vals[5]:
