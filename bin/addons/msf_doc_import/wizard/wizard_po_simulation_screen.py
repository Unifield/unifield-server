...
class wizard_import_po_simulation_screen_line(osv.osv):
    ...
    def import_line(self, cr, uid, ids, values, cc_cache, context=None):
        '''
                        ...
                        if line.type_change in ['new', 'split'] and instance_sync_order_ref:
                            sync_order_label_ids = sync_order_obj.\
                                search(cr, uid, [('name', '=', instance_sync_order_ref),
                                                 ('order_id.state', 'not in', ['done', 'cancel'])], context=context)
                            if sync_order_label_ids:
                                sync_order_label = sync_order_obj.browse(cr, uid, sync_order_label_ids[0], context=context)
                                if sync_order_label.order_id.id == so.id:
                                    write_vals['imp_sync_order_ref'] = sync_order_label.id
                                else:
                                    err_msg = _('This Order in sync. instance reference is not linked with the FO in \'Origin\'')
                                    errors.append(err_msg)
                                    write_vals['type_change'] = 'error'
                            else:
