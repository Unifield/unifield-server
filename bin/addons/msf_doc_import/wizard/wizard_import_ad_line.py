...
class wizard_import_ad_line(osv.osv_memory):
    ...
    def import_file(self, cr, uid, ids, context=None):
        ...
        try:
            ...
            try:
                wb = load_workbook(filename=BytesIO(base64.b64decode(wiz_file)), read_only=True)
                ws = wb.active
            except:
            ...
            for row in rows:
                if len(row) > percentage_col:
                    ...
                    elif row[percentage_col].value and isinstance(row[percentage_col].value, str) and row[percentage_col].value.strip().lower() == 'mix':
                        no_change += 1
                    else:
