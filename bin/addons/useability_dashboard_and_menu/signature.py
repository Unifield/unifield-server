...
class signature_follow_up(osv.osv):
        ...
        cr.execute("""
            create or replace view signature_follow_up as (
                select
                    ...
                    case when jour.type is not null then s.signature_res_model||'.'||jour.type else s.signature_res_model end as doc_type,
                    l.subtype as subtype,
                    count(l.user_id=user_rel.user_id or NULL) as signed,
                    coalesce(po.name, so.name, jour.code|| ' '||per.name) as doc_name,
                ...
                group by
                    user_rel.id, user_rel.user_id, s.signature_res_id, s.signature_state, s.signature_res_model, po.name, so.name, jour.code, jour.type, per.name, l.subtype
            )
        """)
    ...
    _columns = {
        ...
        'doc_type': fields.selection([('purchase.order', 'PO'), ('sale.order', 'IR'), ('account.bank.statement.cash', 'Cash Register'), ('account.bank.statement.bank', 'Bank Register')], 'Document Type', readonly=1),
