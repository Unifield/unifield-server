...
class line_mismatch(XlsxReportParser):
    ...
    def generate(self, context=None):
        ...
        po = self.pool.get('purchase.order').browse(self.cr, self.uid, self.ids[0], fields_to_fetch=['name', 'catalogue_id', 'order_line_mismatch', 'partner_id'], context=context)
        sheet = self.workbook.active

        self.create_style_from_template('title_style', 'A1')
        self.create_style_from_template('header_style', 'A2')
        self.create_style_from_template('col_title', 'A10')
        ...
        sheet.append([self.cell_ro(_('PO Catalogue lines mismatch'), 'title_style')])
        ...
        row_idx += 1
        sheet.merged_cells.ranges.append("A%(row_idx)d:B%(row_idx)d" % {'row_idx': row_idx})
        sheet.append([self.cell_ro(_('Report generated:'), 'header_style'), self.cell_ro('', 'header_style'), self.cell_ro(date_obj.get_date_formatted(self.cr, self.uid, datetime=datetime.now().strftime('%Y-%m-%d')), 'header_style')])
        row_idx += 1
        sheet.append([])
        sheet.append([])
        row_idx += 2
