    ...
    def generate_invoice(self, cr, uid, ids, context=None):
        ...
        for inv_id in all_taxes:
            for key in all_taxes[inv_id]:
                tax_id = self.pool.get('account.invoice.tax').search(cr, uid, [('invoice_id', '=', inv_id), ('account_tax_id', '=', key[0]), ('account_tax_id', '=', key[1]), ('partner_id', '=', key[2])], context=context)
                if not tax_id:
                    self.pool.get('account.invoice.tax').create(cr, uid, {
                            'invoice_id': inv_id,
                            'account_tax_id':  key[0],
                            'account_tax_id': key[1],
                            'partner_id': key[2],
