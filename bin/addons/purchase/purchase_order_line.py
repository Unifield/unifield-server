...
class purchase_order_line(osv.osv):
    ...
    def _get_catalog_mismatch(self, cr, uid, ids, field_name=None, arg=None, context=None):
        """
        TODO: not used
        """
        ret = {}
        ...
        if ids:
            ...
            select
                pol.id,
                pol.price_unit,
                catl.cat_unit_price * coalesce(po_rate.rate,1) / coalesce(cat_rate.rate, 1),
                case when catl.soq_rounding!=0 then pol.product_qty%%catl.soq_rounding=0 else 't' end as match_soq
            from purchase_order_line pol
                left join purchase_order po on po.id = pol.order_id
                left join product_pricelist curr_pricelist on curr_pricelist.id = po.pricelist_id
                join lateral (
                    select
                        cat.currency_id as cat_currency_id, cat_line.unit_price as cat_unit_price, cat_line.rounding as soq_rounding
                    from
                        supplier_catalogue cat
                        left join supplier_catalogue_line cat_line on cat_line.catalogue_id = cat.id and cat_line.product_id = pol.product_id and cat_line.line_uom_id = pol.product_uom
                    where
                        ...
                        cat.period_from < NOW() and
                        (cat.period_to is null or cat.period_to > NOW()) and
                        cat_line.min_qty <= pol.product_qty
                    order by
                        cat.currency_id = curr_pricelist.currency_id, cat_line.min_qty desc, cat_line.id desc
                    limit 1
                ) catl on true
            ...
            for line in cr.fetchall():
                mism = []
                if abs(line[1] - line[2]):
                    mism.append('Unit Price')
                if not line[3]:
                    mism.append('SoQ')
                ret[line[0]] = _(' & '.join(mism))

            cr.execute("select id, catalog_mismatch_frozen from purchase_order_line where id in %s and state in ('confirmed', 'done', 'cancel', 'cancel_r')", (tuple(ids), ))
            for line in cr.fetchall():
                ret[line[0]] = _(line[1])
