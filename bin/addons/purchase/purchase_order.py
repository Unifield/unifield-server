...
from msf_order_date.order_dates import common_create, get_type, common_requested_date_change, \
    common_onchange_transport_lt, common_onchange_date_order, common_onchange_transport_type, common_onchange_partner_id
...
class purchase_order(osv.osv):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for line in order.order_line:
                if line.state not in ('cancel', 'cancel_r'):
                    ...
                    for c in \
                            self.pool.get('account.tax').compute_all(cr, uid, line.taxes_id, line.price_unit,
                                                                     line.product_qty,
    ...
    def _invoiced_rate(self, cursor, user, ids, name, arg, context=None):
        ...
        for purchase in self.browse(cursor, user, ids, context=context):
            if ((purchase.order_type == 'regular' and purchase.partner_id.partner_type in ('internal', 'esc')) or \
                ...
                stock_pickings = sp_obj.search(cursor, user,
                                               [('purchase_id', '=', purchase.id), ('state', '=', 'done')])
                if stock_pickings:
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for line in order.order_line:
                ...
                for move in line.move_ids:
                    if move.state == 'done':
                        move_qty = uom_obj._compute_qty(cr, uid, move.product_uom.id, move.product_qty,
                                                        line.product_uom.id)
                        if move.type == 'in':
    ...
    def _get_order(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get('purchase.order.line').browse(cr, uid, ids, fields_to_fetch=['order_id'],
                                                                context=context):
    ...
    def _get_order_state_changed(self, cr, uid, ids, context=None):
        result = {}
        for line in self.pool.get('purchase.order.line').browse(cr, uid, ids, fields_to_fetch=['order_id', 'state'],
                                                                context=context):
            if line.state in ('cancel', 'cancel_r'):
                result[line.order_id.id] = True
        return result.keys()
    ...
    def _get_requested_date_in_past(self, cr, uid, ids, field_name, args, context=None):
        ...
        for po in self.read(cr, uid, ids, ['delivery_requested_date', 'rfq_ok'], context=context):
            res[po['id']] = po['delivery_requested_date'] and not po['rfq_ok'] and po[
                'delivery_requested_date'] < time.strftime('%Y-%m-%d') or False
    ...
    def _src_customer_ref(self, cr, uid, obj, name, args, context=None):
        ...
        for tu in args:
            if tu[1] in ('ilike', 'not ilike', '=', '!='):
                so_ids = self.pool.get('sale.order').search(cr, uid, [('client_order_ref', tu[1], tu[2])],
                                                            context=context)
                sol_ids = self.pool.get('sale.order.line').search(cr, uid, [('order_id', 'in', so_ids)],
                                                                  context=context)
                pol_ids = self.pool.get('purchase.order.line').search(cr, uid, [('linked_sol_id', 'in', sol_ids)],
                                                                      context=context)
                for pol in self.pool.get('purchase.order.line').browse(cr, uid, pol_ids, context=context):
                    po_ids.add(pol.order_id.id)
            else:
                raise osv.except_osv(_('Error'), _(
                    'Bad operator : You can only use \'=\', \'!=\', \'ilike\' or \'not ilike\' as operator'))
        ...
        for po in self.browse(cr, uid, ids, context=context):
            ...
            if po.empty_po_cancelled:
                ...
                if po.order_line:
                    ...
                    if all([s.startswith('cancel') for s in
                            pol_states]):  # if all lines are cancelled then the PO is cancelled
                        res[po.id] = 'cancel'
                        ...
                        res[po.id] = self.pool.get('purchase.order.line.state').get_less_advanced_state(cr, uid, ids,
                                                                                                        pol_states,
                                                                                                        context=context)

                        if res[po.id] == 'draft':  # set the draft-p state ?
                            draft_sequence = self.pool.get('purchase.order.line.state').get_sequence(cr, uid, ids,
                                                                                                     'draft',
                                                                                                     context=context)
                            # do we have a line further then draft in our FO ?
                            if any([self.pool.get('purchase.order.line.state').get_sequence(cr, uid, ids, s,
                                                                                            context=context) > draft_sequence
                                    for s in pol_states]):
                                res[po.id] = 'draft_p'
                        elif res[po.id] in ('validated', 'validated_n'):  # set the validated-p state ?
                            validated_sequence = self.pool.get('purchase.order.line.state').get_sequence(cr, uid, ids,
                                                                                                         'validated',
                                                                                                         context=context)
                            # do we have a line further then validated in our FO ?
                            if any([self.pool.get('purchase.order.line.state').get_sequence(cr, uid, ids, s,
                                                                                            context=context) > validated_sequence
                                    for s in pol_states]):
                                res[po.id] = 'validated_p'
                            else:
                                res[po.id] = 'validated'
                        elif res[po.id].startswith('sourced'):  # set the sourced-p state ?
                            sourced_sequence = self.pool.get('purchase.order.line.state').get_sequence(cr, uid, ids,
                                                                                                       'sourced',
                                                                                                       context=context)
                            # do we have a line further then sourced in our FO ?
                            if any([self.pool.get('purchase.order.line.state').get_sequence(cr, uid, ids, s,
                                                                                            context=context) > sourced_sequence
                                    for s in pol_states]):
                                res[po.id] = 'sourced_p'
                            else:
                                res[po.id] = 'sourced'
                        elif res[po.id] == 'confirmed':  # set the confirmed-p state ?
                            confirmed_sequence = self.pool.get('purchase.order.line.state').get_sequence(cr, uid, ids,
                                                                                                         'confirmed',
                                                                                                         context=context)
                            # do we have a line further then confirmed in our FO ?
                            if any([self.pool.get('purchase.order.line.state').get_sequence(cr, uid, ids, s,
                                                                                            context=context) > confirmed_sequence
                                    for s in pol_states]):
    ...
    def _is_fixed_type(self, cr, uid, ids, field_name, args, context=None):
        ...
        for po in self.browse(cr, uid, ids, fields_to_fetch=['po_from_fo', 'po_from_ir'], context=context):
            if po.po_from_fo or po.po_from_ir:
                ...
                if sale_ids:
                    for sale in self.pool.get('sale.order').read(cr, uid, sale_ids,
                                                                 ['procurement_request', 'order_type'],
                                                                 context=context):
    ...
    def _get_receipt_date(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            pick_ids = pick_obj.search(cr, uid, [('purchase_id', '=', order.id)], offset=0, limit=1, order='date_done',
                                       context=context)
            if not pick_ids:
                res[order.id] = False
            else:
    ...
    _columns = {
        ...
        'date_order': fields.date(string='Creation Date', required=True, select=True,
        ...
        'name': fields.char('Order Reference', size=64, required=True, select=True, readonly=True,
                            ...
                            sort_column='id'),
        'invoice_ids': fields.many2many('account.invoice', 'purchase_invoice_rel', 'purchase_id', 'invoice_id',
                                        'Invoices', help="Invoices generated for a purchase order", readonly=True),
        'order_line': fields.one2many('purchase.order.line', 'order_id', 'Order Lines', readonly=False),
        'partner_id': fields.many2one('res.partner', 'Supplier', required=True, change_default=True,
        ...
        'invoice_method': fields.selection([('manual', 'Manual'), ('order', 'From Order'), ('picking', 'From Picking')],
        ...
        'allocation_setup': fields.function(_get_allocation_setup, type='selection',
                                            selection=[('allocated', 'Allocated'),
                                                       ...
                                                       ('mixed', 'Mixed')], string='Allocated setup', method=True,
                                            store=False),
        'unallocation_ok': fields.boolean(string='Unallocated PO'),
        'partner_ref': fields.char('Supplier Reference', size=128),
        'short_partner_ref': fields.function(_get_short_partner_ref, method=True, string='Supplier Reference',
                                             type='char', size=64, store=False),
        ...
        'po_from_ir': fields.function(_po_from_x, method=True, type='boolean', string='Is PO from IR ?',
                                      multi='po_from_x'),
        'po_from_fo': fields.function(_po_from_x, method=True, type='boolean', string='Is PO from FO ?',
                                      multi='po_from_x'),
        'canceled_end': fields.boolean(string='Canceled End', readonly=True),
        'is_a_counterpart': fields.boolean('Counterpart?',
                                           help="This field is only for indicating that the order is a counterpart"),
        ...
        'parent_order_name': fields.many2one('purchase.order', string='Parent PO name',
        ...
        'dest_partner_ids': fields.many2many('res.partner', 'res_partner_purchase_order_rel', 'purchase_order_id',
                                             'partner_id', 'Customers'),  # uf-2223
        'dest_partner_names': fields.function(_get_dest_partner_names, type='char', size=256, string='Customers',
                                              method=True),  # uf-2223
        'split_po': fields.boolean('Created by split PO', readonly=True),
        'sourced_references': fields.function(_get_project_ref, method=True, string='Sourced references', type='text',
                                              store=False, multi='so_info'),
        'vat_ok': fields.function(_get_vat_ok, method=True, type='boolean', string='VAT OK', store=False,
                                  readonly=True),
        'requested_date_in_past': fields.function(_get_requested_date_in_past, method=True,
                                                  string='Requested date in past', type='boolean', store=False),
        ...
        'customer_ref': fields.function(_get_customer_ref, fnct_search=_src_customer_ref, method=True,
                                        string='Customer Ref.', type='text', store=False),
        'short_customer_ref': fields.function(_get_short_customer_ref, method=True, string='Customer Ref.', type='text',
                                              store=False),
        ...
        'state': fields.function(_get_less_advanced_pol_state, string='Order State', method=True, type='selection',
                                 selection=PURCHASE_ORDER_STATE_SELECTION, readonly=True,
                                 store={
                                     'purchase.order': (
                                         lambda obj, cr, uid, ids, c={}: ids, ['empty_po_cancelled'], 10),
        ...
        'picking_ids': fields.one2many('stock.picking', 'purchase_id', 'Picking List', readonly=True,
        ...
        'invoiced': fields.function(_invoiced, method=True, string='Invoiced', type='boolean',
                                    help="It indicates that an invoice has been generated"),
        'invoiced_rate': fields.function(_invoiced_rate, method=True, string='Invoiced', type='float'),
        'minimum_planned_date': fields.function(_minimum_planned_date, fnct_inv=_set_minimum_planned_date, method=True,
        ...
        'amount_untaxed': fields.function(_amount_all, method=True, digits_compute=dp.get_precision('Purchase Price'),
                                          ...
                                          store={
                                              'purchase.order.line': [
                                                  (_get_order,
                                                   ['price_subtotal', 'taxes_id', 'price_unit', 'product_qty',
                                                    'product_id'], 10),
        ...
        'amount_tax': fields.function(_amount_all, method=True, digits_compute=dp.get_precision('Purchase Price'),
                                      ...
                                      store={
                                          'purchase.order.line': [
                                              (_get_order, ['price_subtotal', 'taxes_id', 'price_unit', 'product_qty',
                                                            'product_id'], 10),
                                              (_get_order_state_changed, ['state'], 10),
                                          ]
        ...
        'fixed_order_type': fields.function(_is_fixed_type, method=True, type='char', size=256,
                                            string='Possible order types', store={
        ...
        'arrival_date': fields.date(string='Arrival date in the country',
                                    help='Date of the arrival of the goods at custom'),
        ...
        'commitment_ids': fields.one2many('account.commitment', 'purchase_id', string="Commitment Vouchers",
                                          readonly=True),
        ...
        'empty_po_cancelled': fields.boolean('Empty PO cancelled',
                                             help='Flag to see if the PO has been cancelled while empty'),
        'from_address': fields.many2one('res.partner.address', string='From Address', required=True),
        'msg_big_qty': fields.function(_get_msg_big_qty, type='char', string='Lines with 10 digits total amounts',
                                       method=1),
    }
    _defaults = {
        ...
        'loan_duration': 2,
        'invoice_address_id': lambda obj, cr, uid, ctx: obj.pool.get('res.partner').address_get(cr, uid, obj.pool.get(
            'res.users').browse(cr, uid, uid, ctx).company_id.partner_id.id, ['invoice'])['invoice'],
        'invoice_method': lambda *a: 'picking',
        'dest_address_id': lambda obj, cr, uid, ctx: obj.pool.get('res.partner').address_get(cr, uid, obj.pool.get(
            'res.users').browse(cr, uid, uid, ctx).company_id.partner_id.id, ['delivery'])['delivery'],
        'from_address': lambda obj, cr, uid, ctx: obj.pool.get('res.partner').address_get(cr, uid, obj.pool.get(
            'res.users').browse(cr, uid, uid, ctx).company_id.partner_id.id, ['default'])['default'],
        ...
        'shipped': 0,
        'invoiced': 0,
        'partner_address_id': lambda self, cr, uid, context: context.get('partner_id', False) and
                                                             self.pool.get('res.partner').address_get(cr, uid, [
                                                                 context['partner_id']], ['default'])['default'],
    ...
    def _check_order_type(self, cr, uid, ids, context=None):
        ...
        for order in self.read(cr, uid, ids, ['name', 'fixed_order_type', 'order_type', 'is_a_counterpart'],
            ...
            if order['is_a_counterpart'] and order['order_type'] != 'loan':
                err.append(_('%s: This purchase order is a loan counterpart. You cannot change its order type') % order[
                    'name'])
            else:
                ...
                if json_info and order['order_type'] not in json_info:
                    ...
                    err.append(_('%s: Only %s order types are allowed for this purchase order') % (
                        order['name'], allowed_type))
        ...
        if vals.get('order_type'):
            if vals.get('order_type') in ['donation_exp', 'donation_st']:
                ...
                if vals.get('partner_id'):
                    if self.pool.get('res.partner').read(cr, uid, vals.get('partner_id'), ['partner_type'],
                                                         context=context)['partner_type'] == 'esc':
                        vals.update({'invoice_method': 'manual'})
            else:
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for order in self.read(cr, uid, ids, ['partner_id', 'warehouse_id'], context=context):
            partner_type = \
                res_partner_obj.read(cr, uid, int(vals.get('partner_id', order['partner_id'][0])), ['partner_type'],
            ...
            if vals.get('order_type'):
                if vals.get('order_type') in ['donation_exp', 'donation_st']:
                    ...
                    vals.update({'invoice_method': 'picking'})
            # we need to update the location_id because it is readonly and so does not pass in the vals of create and write
            vals = self._get_location_id(cr, uid, vals, warehouse_id=vals.get('warehouse_id', order['warehouse_id'] and
                                                                              order['warehouse_id'][0] or False),
                                         context=context)
        ...
        if vals.get('order_type') and vals['order_type'] in ['loan', 'donation_exp', 'donation_st', 'in_kind']:
            ...
            draft_po_ids = self.search(cr, uid,
                                       [('id', 'in', ids), ('state', '=', 'draft'), ('is_a_counterpart', '=', False)],
                                       context=context)
            if draft_po_ids and pol_obj.search_exist(cr, uid,
                                                     [('order_id', 'in', draft_po_ids), ('origin', '!=', False),
                                                      ('state', '!=', 'cancel')], context=context):
                raise osv.except_osv(
                    _('Error'),
                    _(
                        'You can\'t change the Order Type to Loan, Donation before expiry, Standard donation or In Kind Donation if a line has a Source Document')
                )
    ...
    def unlink(self, cr, uid, ids, context=None):
        if self.get_so_ids_from_po_ids(cr, uid, ids, context=context):
            raise osv.except_osv(_('Error'), _(
                'You cannot remove a Purchase order that is linked to a Field Order or an Internal Request. Please cancel it instead.'))
        ...
        for s in purchase_orders:
            if s['state'] in ['draft', 'cancel']:
                ...
                raise osv.except_osv(_('Invalid action !'),
                                     _('Cannot delete Purchase Order(s) which are in %s State!') % _(
                                         dict(PURCHASE_ORDER_STATE_SELECTION).get(s['state'])))
    ...
    def copy(self, cr, uid, p_id, default=None, context=None):
        ...
        if new_id:
            ...
            if data['order_line'] and data['delivery_requested_date']:
                self.pool.get('purchase.order.line').write(cr, uid, data['order_line'],
                                                           {'date_planned': data['delivery_requested_date']})
    ...
    def copy_data(self, cr, uid, id, default=None, context=None):
        ...
        if context is None:
            context = {}
        fields_to_reset = ['delivery_requested_date', 'ready_to_ship_date', 'date_order', 'delivery_confirmed_date',
                           'arrival_date', 'shipment_date', 'arrival_date', 'date_approve', 'analytic_distribution_id',
                           'empty_po_cancelled', 'stock_take_date']
        to_del = []
    ...
    def onchange_requested_date(self, cr, uid, ids, part=False, date_order=False, requested_date=False, transport_lt=0,
                                order_type=False, context=None):
        '''
        ...
        res = common_requested_date_change(self, cr, uid, ids, part=part, date_order=date_order,
                                           requested_date=requested_date, transport_lt=transport_lt,
                                           type=get_type(self), res=res, context=context)
        return res
    ...
    def onchange_transport_lt(self, cr, uid, ids, requested_date=False, transport_lt=0, context=None):
        ...
        res = {}
        # compute ready to ship date
        res = common_onchange_transport_lt(self, cr, uid, ids, requested_date=requested_date, transport_lt=transport_lt,
                                           type=get_type(self), res=res, context=context)
        return res
    ...
    def onchange_date_order(self, cr, uid, ids, part=False, date_order=False, transport_lt=0, context=None):
        ...
        res = common_onchange_date_order(self, cr, uid, ids, part=part, date_order=date_order,
                                         transport_lt=transport_lt, type=get_type(self), res=res, context=context)
        return res

    def onchange_transport_type(self, cr, uid, ids, part=False, transport_type=False, requested_date=False,
                                context=None):
        '''
        ...
        if context is None:
            context = {}
        res = {}
        res = common_onchange_transport_type(self, cr, uid, ids, part=part, transport_type=transport_type,
                                             requested_date=requested_date, type=get_type(self), res=res,
                                             context=context)
        return res
    ...
    def action_invoice_get_or_create(self, cr, uid, ids, context=None):
        ...
        for o in self.browse(cr, uid, ids):
            if o.id not in po_to_inv:
                ...
                po_to_inv[o.id] = self.pool.get('account.invoice').create(cr, uid, inv_data, {'type': 'in_invoice',
    ...
    def _copy_line_value(self, cr, uid, *args, **kwargs):
        ...
        if order_line.analytic_distribution_id:
            ...
            distrib_id = self.pool.get('analytic.distribution').copy(cr, uid,
                                                                     order_line.order_id.analytic_distribution_id.id)
    ...
    def _hook_order_infos(self, cr, uid, *args, **kwargs):
        ...
        if not order_infos.get('delivery_requested_date') or delivery_requested_date < order_infos[
    ...
    def do_merge(self, cr, uid, ids, context=None):
        ...
        * Order lines are exactly the same except for the quantity and unit
        ...
        for order_key, (order_data, old_ids) in new_orders.iteritems():
            ...
            self.log(cr, uid, neworder_id, _('PO %s have been merged into %s.') % (
                ', '.join([old_po_name.get(x, '') for x in old_ids]),
                self.read(cr, uid, neworder_id, ['name'])['name']),
                     action_xmlid='purchase.purchase_form_action')
            return_info.update({neworder_id: old_ids})

            # make triggers pointing to the old orders point to the new order
            for old_po in self.pool.get('purchase.order').browse(cr, uid, old_ids, fields_to_fetch=['order_line'],
                                                                 context=context):
                reset_linked_sol_id = []
                cancel_pol = []
                ...
                if reset_linked_sol_id:
                    self.pool.get('purchase.order.line').write(cr, uid, reset_linked_sol_id, {'linked_sol_id': False},
                                                               context=context)
                if cancel_pol:
    ...
    def purchase_cancel(self, cr, uid, ids, context=None):
        ...
        if context.get('rfq_ok', False):
            ...
            view_id = \
                data_obj.get_object_reference(cr, uid, 'purchase_override', 'purchase_order_cancel_wizard_form_view')[1]
    ...
    def _check_user_company(self, cr, uid, company_id, context=None):
        ...
        '''
        user_company_id = self.pool.get('res.users').read(cr, uid, uid, ['company_id'], context=context)['company_id'][
            0]
    ...
    def onchange_internal_type(self, cr, uid, ids, order_type, partner_id, categ, dest_partner_id=False,
                               warehouse_id=False, delivery_requested_date=False):
        '''
        ...
        if ids:
            ...
            for order in self.read(cr, uid, ids, ['name', 'fixed_order_type', 'order_type', 'is_a_counterpart']):
                if order['is_a_counterpart'] and order_type != 'loan':
                    err.append(
                        _('%s: This purchase order is a loan counterpart. You cannot change its order type') % order[
                            'name'])
                else:
                    json_info = json.loads(order['fixed_order_type'])
                    if json_info and order_type not in json_info:
                        allowed_type = ' / '.join([_(order_types_dict.get(x)) for x in json_info])
                        err.append(
                            _('%s: Only %s order types are allowed for this purchase order') % (
                                order['name'], allowed_type))
        ...
        if order_type == 'direct':
            if not self.pool.get('purchase.order.line').search_exist(cr, uid, [('order_id', 'in', ids),
                                                                               ('linked_sol_id', '!=', False)]):
        ...
        if partner and partner_id != local_market:
            if partner['partner_type'] in ('internal', 'esc') and order_type in ('regular', 'direct'):
                ...
                w.update({
                    'message': 'You cannot have a Direct Purchase Order with a partner which is not external or an ESC',
    ...
    def onchange_partner_id(self, cr, uid, ids, part=False, date_order=False, transport_lt=False, context=None, *a,
                            **b):
        '''
        ...
        res = {'value': {'partner_address_id': addr['default'], 'pricelist_id': pricelist,
        ...
        if ids:
            ...
            for order in self.browse(cr, uid, ids):
                for line in order.order_line:
                    if line.product_id:
                        res, test = product_obj._on_change_restriction_error(cr, uid, line.product_id.id,
                                                                             field_name='partner_id', values=res,
                                                                             vals={'partner_id': part})
                        if test:
        ...
        if partner['partner_type'] in ('internal', 'esc'):
            ...
            cr.execute('''select po.id from purchase_order po
                ...
                where po.id in %s and (cl1.analytic_id!=%s or cl2.analytic_id!=%s)''',
                       (tuple(ids), intermission, intermission))
            if cr.rowcount > 0:
        ...
        res = common_onchange_partner_id(self, cr, uid, ids, part=part.id, date_order=date_order,
                                         transport_lt=transport_lt, type=get_type(self), res=res, context=context)
    ...
    def onchange_warehouse_id(self, cr, uid, ids, warehouse_id, order_type, dest_address_id):
        ...
        if not warehouse_id:
            ...
            res = {'value': {'location_id': wh_info[0]['lot_input_id'][0], 'dest_address_id': False,
                             'cross_docking_ok': False}}

        if not res.get('value', {}).get('dest_address_id') and order_type != 'direct':
            cp_address_id = self.pool.get('res.partner').address_get(cr, uid, self.pool.get('res.users').browse(cr, uid,
                                                                                                                uid).company_id.partner_id.id,
                                                                     ['delivery'])['delivery']
            if 'value' in res:
    ...
    def check_analytic_distribution(self, cr, uid, ids, context=None, create_missing=False):
        ...
        if po_line_ids:
            return pol_obj.check_analytic_distribution(cr, uid, po_line_ids, context=context,
    ...
    def compute_confirmed_delivery_date(self, cr, uid, ids, confirmed, prep_lt, ship_lt, est_transport_lead_time,
                                        db_date_format, context=None):
        '''
    ...
    def create_new_int_line(self, cr, uid, internal_id, pol, incoming_move_id=False, context=None):
        ...
        elif self.pool.get('stock.location').chained_location_get(cr, uid, src_location, product=pol.product_id,
                                                                  context=context):
            # if input location has a chained location then use it
            dest = self.pool.get('stock.location').chained_location_get(cr, uid, src_location, product=pol.product_id,
        ...
        if isinstance(warehouse_id, (str, unicode)):
            try:
                ...
                raise osv.except_osv(
                    _('Error'),
                    _(
                        'The field \'warehouse_id\' is a float field but value is a string - Please contact your administrator'),
                )

        if not vals.get('cross_docking_ok', False):
            vals.update({'location_id':
                             stock_warehouse_obj.read(cr, uid, warehouse_id, ['lot_input_id'], context=context)[
                                 'lot_input_id'][0]})
    ...
    def set_manually_done(self, cr, uid, ids, all_doc=True, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            if order.loan_id and order.loan_id.state not in ('cancel', 'done') and not context.get('loan_id',
                                                                                                   False) == order.id:
        ...
        if all_doc:
            ...
            for order_id in self.browse(cr, uid, ids, context=context):
                if order_id.tender_id:
                    raise osv.except_osv(_('Error'), _(
                        'You cannot \'Close\' a Request for Quotation attached to a tender. Please make the tender %s to \'Closed\' before !') % order_id.tender_id.name)
    ...
    def check_empty_po(self, cr, uid, ids, context=None):
        ...
        for po in self.browse(cr, uid, ids, context=context):
            if all(x.state in ('cancel', 'done') for x in po.order_line):
                ...
                if po.rfq_ok:
                    view_id = data_obj.get_object_reference(cr, uid, 'tender_flow', 'ask_rfq_cancel_wizard_form_view')[
                        1]
                else:
                    view_id = \
                        data_obj.get_object_reference(cr, uid, 'purchase_override', 'ask_po_cancel_wizard_form_view')[1]
    ...
    def generate_po_from_rfq(self, cr, uid, ids, context=None):
        ...
        new_po_id = self.copy(cr, uid, ids[0], {'name': False, 'rfq_ok': False, 'origin': po_orign},
                              context=dict(context, keepOrigin=True))
        ...
        self.log(cr, uid, new_po_id,
                 _('The Purchase Order %s has been generated from Request for Quotation.') % new_po_data['name'])
    ...
    def create_commitment_voucher_from_po(self, cr, uid, ids, cv_date, context=None):
        ...
        for po in self.pool.get('purchase.order').browse(cr, uid, ids, context=context):
            ...
            view_ids = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'analytic_distribution',
                                                                           'account_commitment_form')
            self.pool.get('account.commitment').log(cr, uid, commit_id, message,
                                                    context={'view_id': view_ids and view_ids[1] or False})
            self.infolog(cr, uid, message)

            # Add analytic distribution from purchase
            if po.analytic_distribution_id:
                new_distrib_id = self.pool.get('analytic.distribution').copy(cr, uid, po.analytic_distribution_id.id,
                                                                             {}, context=context)
                ...
                if new_distrib_id:
                    self.pool.get('analytic.distribution').write(cr, uid, [new_distrib_id],
                                                                 {'purchase_id': False, 'commitment_id': commit_id},
                                                                 context=context)
                    ...
                    self.pool.get('account.commitment').write(cr, uid, [commit_id],
                                                              {'analytic_distribution_id': new_distrib_id},
                                                              context=context)
        ...
        for po in self.browse(cr, uid, ids, context=context):
            ...
            if po.commitment_ids:
                for com in po.commitment_ids:
                    if com.type != 'manual':
                        self.pool.get('account.commitment').action_commitment_done(cr, uid,
                                                                                   [x.id for x in po.commitment_ids],
                                                                                   context=context)
        return True
