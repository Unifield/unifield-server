...
class supplier_catalogue(osv.osv):
    ...
    _columns = {
        ...
        'partner_id': fields.many2one('res.partner', string='Partner', required=True, domain=[('supplier', '=', True)], select=1),
...
class supplier_catalogue_line(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('catalogue_id'):
            catalogue = cat_obj.read(cr, uid, vals['catalogue_id'], ['state', 'active', 'from_sync', 'ranking'], context=context)
