...
class account_journal(osv.osv):
    ...
    _columns = {
        ...
        'type': fields.selection([('sale', 'Sale'),('sale_refund','Sale Refund'), ('purchase', 'Purchase'), ('purchase_refund','Purchase Refund'), ('cash', 'Cash'), ('bank', 'Bank and Cheques'), ('other_reg', 'Other fin institutions/mobile money'), ('general', 'General'), ('situation', 'Opening/Closing Situation')], 'Type', size=32, required=True, select=1,
    ...
    def _check_journal_constraints(self, cr, uid, journal_ids, context=None):
        ...
        if not context.get('sync_update_execution'):
            ...
            for journal in self.browse(cr, uid, journal_ids, fields_to_fetch=fields_list, context=context):
                ...
                if journal_type in ['bank', 'cash', 'cheque', 'cur_adj', 'other_reg']:
                    if not journal.default_debit_account_id or not journal.default_credit_account_id:
                        raise osv.except_osv(_('Warning'),
                                             _('Default Debit and Credit Accounts are mandatory for the journal %s.') % journal_code)
                # check on currency
                if journal_type in ['bank', 'cash', 'cheque', 'other_reg']:
    ...
    def _remove_unnecessary_links(self, cr, uid, vals, journal_id=None, context=None):
        ...
        if not context.get('sync_update_execution'):
            ...
            if journal_type not in ['bank', 'other_reg']:
        ...
        if 'is_active' in vals and not vals['is_active']:
            for journal in self.browse(cr, uid, ids, fields_to_fetch=['type', 'code', 'is_active'], context=context):
                if not journal.is_active:  # skip the checks if the journal is already inactive
                    continue
                if journal.type in ['bank', 'cheque', 'cash', 'other_reg']:  # liquidity journals
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        for journal in self.browse(cr, uid, ids, context=context):
            ...
            if not journal.is_current_instance and not journal.is_coordo_editable \
                    ...
                    and set(vals.keys()) - set(['last_register_date', 'first_register_date']):
    ...
    def onchange_type(self, cr, uid, ids, type, currency, context=None):
        ...
        type_map = {
            ...
            'other_reg':'account_journal_bank_view',
