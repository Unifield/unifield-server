        ...
        for i in invoice_move_lines:
            if inv.currency_id.id != company_currency:
                ...
                i['price'] = cur_obj.compute(cr, uid, inv.currency_id.id,
                                             ...
                                             context={'date': inv.document_date or time.strftime('%Y-%m-%d')})
            else:
    ...
    def move_line_get(self, cr, uid, invoice_id, context=None):
        ...
        for line in inv.invoice_line:
            ...
            for tax in tax_obj.compute_all(cr, uid, line.invoice_line_tax_id,
                ...
                res[-1]['tax_amount'] = cur_obj.compute(cr, uid, inv.currency_id.id, company_currency, tax_amount, context={'date': inv.document_date})
        return res
...
class account_invoice_tax(osv.osv):
    ...
    def base_change(self, cr, uid, ids, base, currency_id=False, company_id=False, document_date=False):
        cur_obj = self.pool.get('res.currency')
        ...
        if currency_id and company_currency:
            base = cur_obj.compute(cr, uid, currency_id, company_currency, base*factor, context={'date': document_date or time.strftime('%Y-%m-%d')}, round=False)
        return {'value': {'base_amount':base}}

    def amount_change(self, cr, uid, ids, amount, currency_id=False, company_id=False, document_date=False):
        cur_obj = self.pool.get('res.currency')
        company_obj = self.pool.get('res.company')
        company_currency = False
        factor = 1
        ...
        if company_id:
            company_currency = company_obj.read(cr, uid, [company_id], ['currency_id'])[0]['currency_id'][0]
        if currency_id and company_currency:
            amount = cur_obj.compute(cr, uid, currency_id, company_currency, amount*factor, context={'date': document_date or time.strftime('%Y-%m-%d')}, round=False)
        ...
        for line in inv.invoice_line:
            for tax in tax_obj.compute_all(cr, uid, line.invoice_line_tax_id, (line.price_unit* (1-(line.discount or 0.0)/100.0)), line.quantity, inv.address_invoice_id.id, line.product_id, inv.partner_id)['taxes']:
                ...
                if inv.type in ('out_invoice','in_invoice'):
                    ...
                    val['base_amount'] = cur_obj.compute(cr, uid, inv.currency_id.id, company_currency, val['base'] * tax['base_sign'], context={'date': inv.document_date or time.strftime('%Y-%m-%d')}, round=False)
                    val['tax_amount'] = cur_obj.compute(cr, uid, inv.currency_id.id, company_currency, val['amount'] * tax['tax_sign'], context={'date': inv.document_date or time.strftime('%Y-%m-%d')}, round=False)
                    val['account_id'] = tax['account_collected_id'] or line.account_id.id
                else:
                    val['base_code_id'] = tax['ref_base_code_id']
                    val['tax_code_id'] = tax['ref_tax_code_id']
                    val['base_amount'] = cur_obj.compute(cr, uid, inv.currency_id.id, company_currency, val['base'] * tax['ref_base_sign'], context={'date': inv.document_date or time.strftime('%Y-%m-%d')}, round=False)
                    val['tax_amount'] = cur_obj.compute(cr, uid, inv.currency_id.id, company_currency, val['amount'] * tax['ref_tax_sign'], context={'date': inv.document_date or time.strftime('%Y-%m-%d')}, round=False)
