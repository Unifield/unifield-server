...
class stock_mission_report_parser(XlsxReportParser):

    def generate(self, cr, uid, data, context):
        ...
        if data.get('file_name') and data['file_name'] == 'consolidate_mission_stock_report.xlsx':
        ...
        if create_missing_report and file_name != 'consolidate_mission_stock_report.xlsx':
            ...
            msr_obj._get_export(cr, uid, report_id, {}, with_valuation=with_valuation, all_products=not display_only_in_stock,
        ...
        if store_in_db:
            ...
            if os.path.exists(path):
                ...
                raise osv.except_osv(_('Error'), _("File %s not found.\nMaybe you need to update the Mission Stock Report data.") % path)


XlsxReport('report.report_mission_stock_report', parser=stock_mission_report_parser, template='addons/mission_stock/report/mission_stock_report.xlsx')

XlsxReport('report.report_consolidated_mission_stock_report', parser=stock_mission_report_parser, template='addons/mission_stock/report/consolidated_mission_stock_report.xlsx')
