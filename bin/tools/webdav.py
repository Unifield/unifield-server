...
# original repo: https://github.com/amnong/easywebdav
...
class Client(object):
    def __init__(self, host, port=0, auth=None, username=None, password=None,
                 protocol='http', verify_ssl=True, path=None, cert=None, onedrive=False):
        if not port:
            port = 443 if protocol == 'https' else 80
        self.baseurl = '{0}://{1}:{2}'.format(protocol, host, port)
        if path and not onedrive:
        ...
        if onedrive:
            ...
            if ctx_auth.acquire_token_for_user(username, password):
                ...
                self.cookies = {'FedAuth': ctx_auth.provider.FedAuth, 'rtFa': ctx_auth.provider.rtFa}
                if path:
                    self.cwd = path
            else:
                raise ConnectionFailed(ctx_auth.get_last_error())
    ...
    def _send(self, method, path, expected_code, **kwargs):
        ...
        response = self.session.request(method, url, allow_redirects=False, cookies=self.cookies, **kwargs)
    ...
    def rename(self, old, new):
        expected_codes = 204
        self._send('MOVE', old, expected_codes, headers={'Destination': '%s' % self._get_url(new)})
