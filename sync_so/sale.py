...
class sale_order_sync(osv.osv):
    ...
    def create_so(self, cr, uid, source, po_info, context=None):
        print "+++ Create an FO at %s from a PO (normal flow) at %s"%(cr.dbname, source)
        if not context:
            context = {}
...
#                so_line_obj.write(cr, uid, [line.id], {'confirmed_delivery_date': False})
    ...
    def validated_po_update_validated_so(self, cr, uid, source, po_info, context=None):
        print "+++ Update the validated FO at %s when the relevant PO got validated at %s"%(cr.dbname, source)
        if not context:
            context = {}
        context['no_check_line'] = True
    ...
    def update_sub_so_ref(self, cr, uid, source, po_info, context=None):
        print "+++ Update the PO references from %s to the FO, including its sub-FOs at %s"%(source, cr.dbname)
        if not context:
            context = {}
