...
class purchase_order_sync(osv.osv):
    ...
    def create_split_po(self, cr, uid, source, so_info, context=None):
        ...
        fo_ids = self.pool.get('sale.order').search(cr, uid, [('loan_id', 'in', order_ids)], context=context)
        if fo_ids:
            netsvc.LocalService("workflow").trg_change_subflow(uid, 'sale.order', fo_ids, 'purchase.order', order_ids, res_id, cr)
