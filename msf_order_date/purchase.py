...
class purchase_order(osv.osv):
    ...
    def check_dates(self, cr, uid, data, context={}):
        ...
        if not self.check_date_order(cr, uid, date_order, context=context):
            raise osv.except_osv(_('Error'), _('The creation date is not in an opened period !'))
        if not self.check_delivery_requested(requested_date, context=context):
            raise osv.except_osv(_('Error'), _('The Delivery Requested Date should be between today and today + 24 months !'))
        if not self.check_delivery_confirmed(confirmed_date, date_order, context=context):
            raise osv.except_osv(_('Error'), _('The Delivery Confirmed Date should be older than the Creation date !'))
        if not self.check_delivery_confirmed(ready_to_ship, date_order, context=context):
    ...
    _columns = {
        ...
        'history_date_ids': fields.one2many('purchase.order.date.history', 'order_id', string='History'),
    }
    ...
    def requested_date_change(self, cr, uid, ids, requested_date, confirmed_date, date_order, leadtime=0, context={}):
        ...
        if not self.check_delivery_confirmed(confirmed_date, date_order, context):
            message = {'title': _('Warning'),
    ...
    def onchange_partner_id(self, cr, uid, ids, part):
        ...
        if part:
            ...
            requested_date = requested_date + relativedelta(days=company.po_lead)
