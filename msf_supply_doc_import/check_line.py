...
def quantity_value(**kwargs):
    ...
    try:
        if not row.cells[2]:
            ...
            try:
                product_qty = float(row.cells[2].data)
            except ValueError as e:
                error_list.append('The Product Quantity was not a number and it is required to be greater than 0, it is set to 1 by default. Details: %s.' % e)
    # if the cell is empty
    except IndexError:
        warning_list.append('The Product Quantity was not set. It is set to 1 by default.')
