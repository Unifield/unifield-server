...
class account_model(osv.osv):
    ...
    def _has_any_bad_ad_line_exp_in(self, cr, uid, ids, name, args, context=None):
        return self.__has_any_bad_ad_line_exp(cr, uid, ids, ('no_header', 'invalid'), context=context)

    def __has_any_bad_ad_line_exp(self, cr, uid, ids, states, context=None):
        res = {}
        if not ids:
            return res
        if isinstance(ids, (int, long)):
        ...
        for model in self.browse(cr, uid, ids, context=context):
            ...
            for line in model.lines_id:
                if line.exp_in_ad_state and line.exp_in_ad_state in states:
                    res[model.id] = True
                    break
        return res
