...
class report_budget_actual(report_sxw.report_sxw):
    ...
    def _get_header(self, cr, uid, budget, parameters, context=None):
        pool = pooler.get_pool(cr.dbname)
        result =  [['Budget name:', budget.name],
                   ['Budget code:', budget.code],
                   ['Fiscal year:', budget.fiscalyear_id.name],
                   ['Cost center name:', budget.cost_center_id.name],
                   ['Cost center code:', budget.cost_center_id.code],
    ...
    def _get_lines(self, cr, uid, budget, parameters, context=None):
        if context is None:
            context = {}
        pool = pooler.get_pool(cr.dbname)
        result = []
        ...
        # Cost Center
        cost_center_ids = pool.get('msf.budget.tools')._get_cost_center_ids(budget.cost_center_id)
        general_domain = [('cost_center_id', 'in', cost_center_ids)]
        # Dates
        general_domain.append(('date', '>=', budget.fiscalyear_id.date_start))
        general_domain.append(('date', '<=', budget.fiscalyear_id.date_stop))
        # Update context with currency table
        context.update(parameters)
        # Get actual expenses
        actual_expenses = pool.get('msf.budget.tools')._get_actual_amounts(cr,
                                                                           uid,
                                                                           budget.currency_id.id,
    ...
    def create(self, cr, uid, ids, data, context=None):
        ...
        header_data = self._get_header(cr, uid, budget, data['form'], context)
        line_data = self._get_lines(cr, uid, budget, data['form'], context)
        data = header_data + [['']] + line_data
