        ...
        for reconciled in reconciled_obj.browse(cr, uid, ids, context=context):
            ...
            if addendum_line_ids:
                ...
                for al in self.browse(cr, uid, addendum_line_ids, context=context):
                    ...
                    reconcile_txt = ''
                    r_id = False
                    if al.reconcile_total_partial_id:
                        r_id = al.reconcile_total_partial_id.id
                    elif al.reconcile_id:
                        r_id = al.reconcile_id.id
                    if r_id:
                        data = self.pool.get('account.move.reconcile').name_get(cr, uid, [r_id])
                        if data and data[0] and data[0][1]:
                            reconcile_txt = data[0][1]
                    sql = """
                        UPDATE account_move_line
                        SET debit_currency=%s, credit_currency=%s, amount_currency=%s, debit=%s, credit=%s, reconcile_txt=%s
                        WHERE id=%s
                    """
                    cr.execute(sql, [0.0, 0.0, 0.0, addendum_db or 0.0, addendum_cr or 0.0, reconcile_txt or '', tuple([al.id])])
                    # Update partner line
                    if isinstance(other_line_ids, (int, long)):
                        other_line_ids = [other_line_ids]
                    reconcile_txt = '' # reset reconcile_txt for partner's lines because it's useless
                    for o in self.pool.get('account.move.line').browse(cr, uid, other_line_ids):
                        cr.execute(sql, [0.0, 0.0, 0.0, partner_db or 0.0, partner_cr or 0.0, reconcile_txt or '', tuple([o.id])])
